...
# Sets up environment for building native program on Android.
#
...
if [ ! -d "${ANDROID_NDK_ROOT}" ]; then
  echo "ANDROID_NDK_ROOT must be set to the path of Android NDK." \
    >& 2
  return 1
fi

if [ ! -d "${ANDROID_SDK_ROOT}" ]; then
  echo "ANDROID_SDK_ROOT must be set to the path of Android SDK." \
    >& 2
  return 1
fi
...
android_gyp() {
  "${CHROME_SRC}/build/gyp_test" --depth="${CHROME_SRC}"
}
